...
function useHistData() {
        ...
        .then(({ data, error }) => {
          if (data && !error) {
            ...
            setHistory(localEntries.slice().reverse().map((e, i) => ({
              id: i, created_at: '', date: e.date,
              total_pallets: e.totalPallets, total_bultos: e.totalBultos, tiendas: e.tiendas,
            })));
          }
          setLoading(false);
        });
  ...
  return { history, loading, redownload };
}

function HistEntries({ history, loading, redownload }: {
  history: DispatchRow[];
  loading: boolean;
  ...
  return (
    <div className="p-3.5">
      ...
      {!loading && !history.length && <div className="text-center py-16 text-text-3 text-[15px]">No hay despachos registrados aún.</div>}
      {history.map((entry) => (
        <div key={entry.id} className="bg-white border-[1.5px] border-border rounded-card p-3.5 mb-3 shadow-card">
          ...
          <div className="flex items-center justify-between mt-2.5 pt-2 border-t border-border">
            <div className="text-[13px] text-text-3">
              Total monto: <strong className="text-success font-mono">${entry.tiendas.reduce((s, t) => s + (t.monto || 0), 0).toLocaleString('es-CL')}</strong>
            </div>
...
/** Embeddable content — no page shell, for use inside EstadoScreen Historial tab */
export function HistContent() {
  const { history, loading, redownload } = useHistData();
  return (
