...
const baseItem: SantiagoItem = {
  ...
  ancho: 100,
  pesoVolumetrico: 0,
  regimen: 'Falabella' as SantiagoItem['regimen'],
  orden: 'P1',
  estado: 'Listo para despachar',
} as SantiagoItem;

describe('sheetsSantiago buildRows — [P4] fecha de registro', () => {
  const items = { '32BNV': [baseItem] };
  ...
  it('sin fecha de armado, FECHA cae a hoy', () => {
    const rows = buildRows(items, 'Falabella', despacho);
    const now = new Date();
