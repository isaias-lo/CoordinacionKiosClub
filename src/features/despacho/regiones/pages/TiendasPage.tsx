...
export function TiendasPage({ onRegistrar }: { onRegistrar?: () => void } = {}) {
  ...
  // target, restaura los items previos (source → slot nuevo) y REABRE la tienda para que la
  // reconstrucción rearme las cards deterministamente.
  interface UnionSnap { name: string; itemsAntes: DispatchItem[]; sourceItem: DispatchItem; oldSrcSlot?: number; tgtSlot?: number; tgtPeso: number; }
  const revertirUnificacion = async (snap: UnionSnap) => {
    ...
    setPickingSlotsFull(prev => {
      const arr = [...(prev[name] ?? [])].map(s => (s.id === tgtSlot ? { ...s, peso_kg: tgtPeso } : s));
  ...
  // leería el peso del destino desactualizado entre iteraciones) y se hace UN solo UPDATE_ITEMS,
  ...
  const sumarVariosAPallet = (bultoRowIds: string[], palletRowId: string) => {
    if (!selectedTienda) return;
    const palletRow = formRows.find(r => r.id === palletRowId);
    const bultoRows = formRows.filter(r => bultoRowIds.includes(r.id) && (r.pkg === 'box' || r.pkg === 'chocolate'));
  ...
  const iniciarUnionInline = (sourceRow: FormRow, targetRow: FormRow, srcLabel?: string, tgtLabel?: string) => {
    ...
    if (puedeRevertirUnion && sourceItemUnion) {
      armarUndo(`Unificado ${srcLabel ?? ''} → ${tgtLabel ?? ''}`.replace(/\s+→\s*$/, ''), () => revertirUnificacion({
        name, itemsAntes: itemsAntesUnion, sourceItem: sourceItemUnion, oldSrcSlot: srcSlot, tgtSlot, tgtPeso,
      }));
    }
  };
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        <div ref={isMobile ? formScrollRef : formScrollDesktopRef} className="flex-1 overflow-y-auto px-2 py-2">
                  ...
                  <div key={row.id} className="bg-white rounded-lg border-2 p-2" style={{ borderColor: rowColor.border }}>
                    ...
                    {dupRow === row.id && row.pkg === 'box' && (
                      <div className="mt-1.5 pt-1.5 border-t border-black/[0.08] flex items-center gap-1.5 flex-wrap">
                        <span className="text-[9px] font-bold uppercase tracking-wide text-warn">Duplicar</span>
                        <div className="flex items-center border border-black/[0.15] rounded-md overflow-hidden bg-white">
                          <button onClick={() => setDupN(n => Math.max(1, n - 1))} className="w-6 h-7 text-navy font-bold text-[15px] leading-none">−</button>
                          <span className="w-7 text-center font-bold text-[14px] tabular-nums">{dupN}</span>
                          <button onClick={() => setDupN(n => Math.min(20, n + 1))} className="w-6 h-7 text-navy font-bold text-[15px] leading-none">+</button>
                        </div>
                        <button onClick={() => { const n = dupN; setDupRow(null); void duplicarBulto(row, n); }}
                          className="h-7 px-2.5 rounded-md bg-warn text-white text-[11px] font-bold flex items-center gap-1">⧉ Duplicar {dupN}</button>
                      </div>
                    )}
          ...
          {(() => {
            ...
            const palletRows = formRows.filter(r => r.pkg === 'pallet' && (r.savedItem || r.pickingSlotId));
