...
export function RecepcionTiendaCtrlForm({ qrData, onDone, onBack }: Props) {
  ...
  const handleConfirm = async () => {
    ...
    try {
      ...
      for (let i = 0; i < estadoFiles.length; i++) {
        ...
        estadoFotoUrls.push(await uploadPhoto(f, `${uid}_estado${i + 1}.${ext}`));
      }
      const signatureDataUrl = canvasRef.current!.toDataURL('image/png');

      const res = await fetch('/api/recepcion-tienda', {
        method: 'POST',
  ...
  return (
    <div style={{ paddingBottom: 40, overflowY: 'auto', flex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 14, padding: '16px 16px 0' }}>
        ...
        <div style={S.card}>
          ...
          <p style={{ margin: '0 0 4px', fontSize: 11, fontWeight: 700, color: '#9CA3AF', letterSpacing: '0.1em', textTransform: 'uppercase' }}>Tienda</p>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginTop: 12 }}>
            ...
            {contenedoresSent > 0 && <span style={{ background: 'rgba(107,33,168,0.12)', color: '#6B21A8', fontSize: 13, fontWeight: 700, padding: '4px 12px', borderRadius: 99 }}>{contenedoresSent} contenedor{contenedoresSent !== 1 ? 'es' : ''} enviado{contenedoresSent !== 1 ? 's' : ''}</span>}
          ...
          <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
            {SELLO_OPTS.map(opt => (
              <button key={opt.value} onClick={() => setSelloEstado(opt.value)}
                style={{ flex: 1, padding: '10px 0', borderRadius: 12, border: `2px solid ${selloEstado === opt.value ? opt.color : '#E5E7EB'}`, background: selloEstado === opt.value ? opt.bg : '#fff', color: selloEstado === opt.value ? opt.color : '#6B7280', fontWeight: 700, fontSize: 13, cursor: 'pointer' }}>
                {opt.label}
              </button>
            ))}
          </div>

          <p style={S.label}>Foto del sello <span style={{ fontWeight: 400, color: '#9CA3AF' }}>(opcional — solo cámara)</span></p>
          {selloPreview ? (
        ...
        <div style={S.card}>
          ...
          <label style={{ display: 'flex', alignItems: 'center', gap: 10, padding: '12px 14px', border: '2px dashed #D1D5DB', borderRadius: 12, cursor: 'pointer' }}>
            ...
            <div>
              ...
              <div style={{ fontSize: 11, color: '#9CA3AF', marginTop: 2 }}>Puedes seleccionar múltiples fotos</div>
            </div>
